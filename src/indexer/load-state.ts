...
import type { ChainGraphTableWhitelist } from '../types'
    ...
    Promise.map(
      ...
      async ({ table, scopes }) => {
        ...
        async function fn(scope) {
          ...
          const delphiOracleRows = tableDataDeltas
            ...
            .filter((row) => {
              const filteredRowData = delphioracleProducersFilter.filter(
                (producer) =>
                  producer.producer === row.data.owner &&
              ...
              if (filteredRowData.length === 0) {
                ...
                const producer = delphioracleProducersFilter.find(
                  (producer) => producer.producer === row.data.owner,
                )
            ...
            .map((row) => {
              return {
                ...row,
                id: Number.parseInt(row.data.id.toString(), 10),
